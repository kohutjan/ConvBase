        ...
        for (int hK = 0; hK < this->kernelSize; ++hK)
        {
          if (this->pad)
          {
            for (int wK = 0; wK < this->kernelSize; ++wK)
            {
              ...
              if (insidePad)
              {
                fill(topData + topOffset, topData + topOffset + this->bottomShape[0][Cd], 0.0);
              }
              else
              {
                float * inputPixel = bottom[0].GetPixel(n, h - this->pad + hK, w - this->pad + wK);
                memcpy(topData + topOffset, inputPixel, this->bottomShape[0][Cd] * sizeof(float));
              }
          ...
          else
          {
            ...
            memcpy(topData + topOffset, inputPixel, this->kernelSize * this->bottomShape[0][Cd] * sizeof(float));
...
void Im2Col::Backward(vector<Tensor4D> top, vector<Tensor4D> bottom)
{
